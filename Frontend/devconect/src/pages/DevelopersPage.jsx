...
const DevelopersPage = () => {
  ...
  return (
    <section className="min-h-screen bg-midnight-black py-16 px-4 sm:px-8 lg:px-16 text-white">
      { }
      ...
      <div className="max-w-7xl mx-auto mb-12 space-y-6">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          {commonSkills.filter(s => !selectedSkills.includes(s)).slice(0, 6).map(skill => (
            <button
              ...
              className="px-3 py-1 text-xs rounded-full bg-midnight-gray/50 border border-white/10 text-midnight-blue hover:bg-midnight-blue/20 transition-all"
            >
              + {skill}
            </button>
          ))}
        ...
        {selectedSkills.length > 0 && (
          <div className="flex flex-wrap gap-2">
            {selectedSkills.map(skill => (
              <span
                key={skill}
                className="px-3 py-1.5 rounded-full bg-midnight-blue/20 border border-midnight-blue/30 text-midnight-blue text-sm flex items-center gap-2"
              >
                {skill}
                <X
        ...
        <div className="flex justify-between items-center">
          <p className="text-gray-400 text-sm">
            Found <span className="text-midnight-blue font-semibold">{developers.length}</span> developer{developers.length !== 1 ? 's' : ''}
          </p>
          {hasActiveFilters && (
            <button
            ...
            >
              ...
              <img
                ...
                className="
                  ...
                  group-hover:shadow-[0_0_35px_rgba(59,130,246,0.8)]
                  transition-all duration-300
                "
              />
                ...
                {item.skills && item.skills.length > 0 && (
                  <div className="flex flex-wrap gap-1 mt-2">
                    {item.skills.slice(0, 3).map((skill, idx) => (
                      <span key={idx} className="px-2 py-0.5 text-xs rounded-full bg-midnight-blue/10 border border-midnight-blue/30 text-midnight-blue">
                        {skill}
                      </span>
                    ))}
                ...
                <Link to={`/developers/${item._id}`}>
                  <button
                    className="
                      ...
                      text-white
                      shadow-lg shadow-midnight-blue/40
                      group-hover:shadow-midnight-blue/40
                      hover:scale-[1.05]
